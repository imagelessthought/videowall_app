...
async function displaySlide(slideData, config) {
  ...
  console.log("DEBUG: slideData.launch-wait-class =", slideData["launch-wait-class"]);
  if (slideData["launch-wait-class"] && slideData["launch-wait-class"] !== "null") {
    ...
    if (slideshowParent) {
      ...
      console.log("DEBUG: Created countdownDiv with id 'countdown' and class:", countdownDiv.className);
      slideshowParent.insertBefore(countdownDiv, slideshowParent.firstChild);
      console.log("DEBUG: countdownDiv inserted as first child of slideshow_parent");
    } else {
